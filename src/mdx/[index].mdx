...
export function Count({ count }) {
  return <p>Count from {count}!</p>
}

<Layout title="MDX-Example" css="/mdx/index.css">
 <section>
# Rendering MDX with Jeasx

## This is a subline

<Hello name="Maik" />

Hello from {props.request.url}.

Below is an example of markdown in JSX.

<div style={{"background-color": 'violet', "padding": '1rem'}}>
  Try and change the background color to `tomato`.
</div>


{[1,2,3].map(num => <Count count={num} />)}
